...
class JadwalJagaController extends Controller
{
    ...
    public function store(Request $request)
    {
        try {
            ...
            $request->validate([
                ...
                'asisten_id' => 'required|integer|exists:asistens,id', // Pastikan asisten_id ada di tabel asistens
            ]);

            // Cek apakah asisten sudah ada di jadwal untuk kelas yang sama
            $existingJadwal = JadwalJaga::where('kelas_id', $request->kelas_id)
            ...
            return response()->json([
                'message' => 'Gagal menambahkan jadwal',
                'error' => $e->getMessage(), // Kirim pesan error ke frontend
            ], 500);
        }
    }
    ...
    public function update(Request $request, $id)
    {
        try {
            ...
            // Cek apakah asisten sudah ada di jadwal untuk kelas yang sama (kecuali dirinya sendiri)
            $existingJadwal = JadwalJaga::where('kelas_id', $request->kelas_id)
                ->where('asisten_id', $request->asisten_id)
                ->where('id', '!=', $id)
                ->first();
            ...
            return response()->json(['message' => 'Berhasil memperbarui jadwal'], 200);
        } catch (\Exception $e) {
            return response()->json([
