...
export default function MandiriReview({ modulId, onNavigate }) {
        ...
        const load = async () => {
            ...
            try {
                const [questionResponse, answerResponse] = await Promise.all([
                    api.get(`/api-v1/soal-tm/${modulId}`),
                    api
                        .get(`/api-v1/jawaban-tm/${modulId}`)
                ...
                const answers = Array.isArray(answerResponse?.jawaban_mandiri) ? answerResponse.jawaban_mandiri : [];
                ...
                console.error("Failed to load mandiri review", loadError);
                const message = loadError?.response?.data?.message ?? loadError.message ?? "Gagal memuat review mandiri.";
                setError(message);
                setEntries([]);
            } finally {
                setIsLoading(false);
