...
export default function JurnalReview({ modulId, onNavigate }) {
        ...
        const load = async () => {
            ...
            try {
                const [questionResponse, answerResponse] = await Promise.all([
                    api.get(`/api-v1/soal-jurnal/${modulId}`),
                    api
                        .get(`/api-v1/jawaban-jurnal/${modulId}`)
                ...
                const answers = Array.isArray(answerResponse?.jawaban_jurnal) ? answerResponse.jawaban_jurnal : [];
                ...
                console.error("Failed to load jurnal review", loadError);
                const message = loadError?.response?.data?.message ?? loadError.message ?? "Gagal memuat review jurnal.";
                setError(message);
                setEntries([]);
            } finally {
                setIsLoading(false);
