...
export default function ModalAddPlotting({ onClose, fetchKelas }) {
    ...
    const handleSave = async () => {
        ...
        try {
            setLoading(true);
            // Kirim data kelas ke backend
            const kelasResponse = await axios.post("/api-v1/kelas", formData, {
                headers: {
                    Authorization: `Bearer ${localStorage.getItem("token")}`,
                },
            });

            if (kelasResponse.data.status === 'success') {
                ...
                alert("Gagal menyimpan data kelas: " + kelasResponse.data.message);
            }
        } catch (error) {
            console.error("Failed to save kelas:", error.response?.data);
            alert(`Gagal menyimpan data kelas: ${error.response?.data?.error || error.message}`);
        } finally {
            setLoading(false);
        }
    };
